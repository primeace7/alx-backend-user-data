...
def register_user(email: str, password: str) -> None:
    ...
    response2 = requests.post(
        'http://0.0.0.0:5000/users', data={'email': email, 'password': password})
...
def log_in_wrong_password(email: str, password: str) -> None:
    ...
    response = requests.post(
        'http://0.0.0.0:5000/sessions', data={'email': email, 'password': password})
...
def profile_logged(session_id: str) -> None:
    ...
    response = requests.get('http://0.0.0.0:5000/profile', cookies=cookie)
...
def log_out(session_id: str) -> None:
    ...
    cookie = {'session_id': session_id}
    response = requests.delete('http://0.0.0.0:5000/sessions', cookies=cookie)
...
def update_password(email: str, reset_token: str, new_password: str) -> None:
    ...
    response = requests.post('http://0.0.0.0:5000/reset_password', data=payload)
