...
def register_user(email: str, password: str) -> None:
    ...
    response = requests.post(
        'localhost:5000/users', data={'email': email, 'password': password})

    assert response.status_code == 200
    assert response.json() == {"email": email, "message": "user created"}

    response2 = requests.post(
        '127.0.0.1:5000/users', data={'email': email, 'password': password})
...
def log_in_wrong_password(email: str, password: str) -> None:
    ...
    response = requests.post(
        '127.0.0.1:5000/sessions', data={'email': email, 'password': password})
...
def profile_unlogged() -> None:
    ...
    response = requests.get('127.0.0.1:5000/profile')
...
def profile_logged(session_id: str) -> None:
    ...
    response = requests.get('127.0.0.1:5000/profile', cookies=cookie)
...
def log_out(session_id: str) -> None:
    ...
    cookie = {'session_id': session_id}
    response = requests.delete('127.0.0.1:5000/sessions', cookies=cookie)
...
def reset_password_token(email: str) -> str:
    ...
    response = requests.post(
        '127.0.0.1:5000/reset_password', data={'email': email})
...
def update_password(email: str, reset_token: str, new_password: str) -> None:
    ...
    response = requests.post('127.0.0.1:5000/reset_password', data=payload)
